...
-- essa função só grava o que recebe.
-- Rodar uma vez em: Supabase → SQL Editor → New query → colar tudo → Run.

alter table public.viagens
  add column if not exists data_retorno date,
  add column if not exists horario_retorno time,
  add column if not exists origem_retorno_endereco text,
  add column if not exists destino_retorno_endereco text;
